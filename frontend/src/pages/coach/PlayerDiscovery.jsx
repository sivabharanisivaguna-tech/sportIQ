...
import { Search, User, Eye, Activity, BrainCircuit, Video, CheckCircle2, Play } from 'lucide-react';
...
export const PlayerDiscovery = () => {
  ...
  const [videoModalTitle, setVideoModalTitle] = useState('Video Proof');
  ...
  const fetchPlayers = async () => {
    try {
      ...
      const res = await coachService.listSquadPlayers(params);
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-[11px] text-[#94A3B8]">{p.age || '--'} yrs • {playerEmail || p.gender || 'N/A'}</span>
            </div>
          </div>
        );
      }
  ...
  return (
    <div className="space-y-6">
      <div className="pb-4 border-b border-[#1E293B]">
        <h1 className="text-2xl font-bold text-[#F8FAFC] font-display tracking-tight">Player Talent Discovery</h1>
        <p className="text-xs text-[#94A3B8]">Search and evaluate athlete pool across disciplines, examine physical metrics, and review AI talent ratings</p>
      </div>
      ...
      {/* Results Table */}
      <Card title={`Discovered Athletes (${total})`}>
        {loading ? (
          <Loader message="Querying athlete directory..." className="py-12" />
        ) : players.length > 0 ? (
              ...
              <div className="p-8 rounded-2xl bg-[#0B1220] border border-[#1E293B] text-center">
                ...
                <Button
                  onClick={() => {
                    ...
                    setSelectedPlayer(inspectPlayer);
                  }}
                  variant="primary"
                  size="sm"
                >
      ...
      <RecommendationModal
        ...
        playerName={selectedPlayer?.name || selectedPlayer?.user?.name}
